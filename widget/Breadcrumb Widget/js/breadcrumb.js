...
function($, CCConstants, ko, CCLogger, pubsub, CCi18n) {
  ...
  return {
	...
	$.Topic(pubsub.topicNames.SEARCH_RESULTS_UPDATED).subscribe(function(data){
	    /*if(data.hasOwnproperty('parameters')){
	        if(widget.getParam(widget.currentPage.parameters, 'Ntt') !== null) widget.isCategory(false);
	        widget.doBreadcrumb(widget.currentPage);
	    };*/
    ...
    doBreadcrumb : function(page) {
      ...
      widget.getCustomProductCategories(page);
    },
    getCustomProductCategories: function(getPage) {
        var widget = this;
        if(widget.isProductPage(getPage)) {
            var breadcrumb = widget.breadcrumb();
            var finalBreadcrumb = breadcrumb.slice(1);
            var tempBreadText = "";
            var tempBread = [];
            for(var i=0; i < finalBreadcrumb.length; i++){
                tempBread.push(finalBreadcrumb[i].label);
                tempBreadText+= tempBread[i] + " > "
            }
            var lastIndex = tempBreadText.lastIndexOf(">");
            tempBreadText = tempBreadText.substring(0, lastIndex);
            $.Topic('bronotoBreadcrumb.memory').publish(tempBreadText);
          }
    },
    /*
    ...
    //$.Topic('bronotoBreadcrumb.memory').publish(currentCat);
