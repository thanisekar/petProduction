    ...
    return {
		...
		onLoad: function(widget) {
			...
			function checkAndCacheData(refreshData) {
				...
				if(refreshData || !storageApi.getInstance().getItem("birdCarouselData")) {
					var dataUrl = ""	;
				   var url = window.location.hostname;
					...
					var getCatData = JSON.parse(storageApi.getInstance().getItem("birdCarouselData"))
		...
		getStockPrice: function(getData) {
                ...
                i.authenticatedRequest("/ccstoreui/v1/prices/products?ids="+getProductIds.toString(), {}, function(data) {
                   ...
                   storageApi.getInstance().setItem("birdCarouselData", JSON.stringify(getData));
        ...
        updateFeatureData: function(getData) {
               var salePrice;
                ...
                getBirdData ='';
                getBirdData=getCarouselWidget.koDealsModelData();
              $.Topic("HOME_BIRD_BV").publish(getBirdData);
            }
    };
  }
);
