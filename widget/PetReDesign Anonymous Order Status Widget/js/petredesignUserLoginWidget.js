define(
  ...
  // DEPENDENCIES
  //-------------------------------------------------------------------
  ['jquery', 'knockout', 'navigation', 'notifier', 'ccPasswordValidator', 'pubsub', 'CCi18n', 
   'ccConstants', 'navigation', 'ccLogger', 'spinner','ccResourceLoader!global/PetReDesignValidationWidget'],
    
  //-------------------------------------------------------------------
  // MODULE DEFINITION
  //-------------------------------------------------------------------
  function ($, ko, n, notifier, CCPasswordValidator, pubSub, CCi18n, CCConstants,
    ...
    return {
      ...
      getOrderDetails: function(data, event) {
             getWidget.createSpinner();
                ...
                var orderId = getWidget.orderno().trim();
                if(orderId.charAt(0) == 'O'){
                    orderId = getWidget.orderno().trim().toLowerCase();        
                }
                else if(orderId.charAt(0) == 'o'){
                    orderId = getWidget.orderno().trim().toLowerCase();        
                }
                else if(orderId.charAt(0) != 'O'){
                    orderId = getWidget.orderno().trim().toUpperCase();        
                }
                       ...
                       //getWidget.createSpinner();
                        ...
                        $.ajax({ //  รง Ajax call to getorder Api
                            ...
                            success: function(order) {
                                ...
                                //window.location = '/guestorderdetails/'+orderId + ',' + getWidget.orderEmail().trim();
                                n.goTo('/guestorderdetails/'+orderId + ',' + getWidget.orderEmail().trim());
                                //getWidget.destroySpinner();
                            },
