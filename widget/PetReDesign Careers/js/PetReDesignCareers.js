...
define(
    ...
    function($, ko) {
        ...
        return {
		    ...
		    //document.createElement('<div id="jobs-widget" style="width: 100%; height: 100%;"><div style="border:1px solid #e3e3e3; padding: 30px; -moz-border-radius:5px;border-radius:5px;-webkit-border-radius:5px; text-align: center; margin-bottom: 7px;"><img style="margin-left:auto; margin-right: auto;" src="/zrs/80eb026d/img/ajax-loader-sm.gif" /><br />Loading jobs...</div></div>');
            document.close();
            var zrdebug = false,
                zrlog = function(msg) {
            ...
            zrHead.ready(document, function() {
                ...
                function doAjaxReq(params) {
                    ...
                    if (!jqInitAttempted) {
                        ...
                        jq = !params.customJquery ? (window.jQuery || window.$) : jQuery.noConflict();
                        if (jq) {
                    ...
                    jq.ajax({
                        url: "https:\/\/www.ziprecruiter.com\/jobs-widget-content\/v1\/889f3581\/all",
                        method: "GET",
                        cache: false,
                        dataType: "html",
                        ...
                        error: function(xhr, status, error) {
                            ...
                            // There are cases where IE in compatibility mode (IE11) doesn't report as such and we need to fall back to a CORS plugin when there is an access error
                                ...
                                if (!prepareRequest.stop) {
                                    zrlog("Access denied error in IE - attempting to use xDomain plugin");
                                    prepareRequest.stop = true;
                                    prepareRequest({
                                        forceXdomainPlugin: true
                                    });
                                }
                            }
                        }
                ...
                var sortByDaysAgo = function(ltVal, daysAgoTextA, daysAgoTextB) {
                    ...
                    return ((norm.daysAgoA < norm.daysAgoB) ? ltVal : ((norm.daysAgoA > norm.daysAgoB) ? gtVal : 0));
                }
            
            
                function showJobsData(data) {
                    zrHead.load('https://www.ziprecruiter.com/js/vendor/dataTables/1.10.0/jquery.dataTables.js', function() {
                        jq('#jobs-widget').html(data);
                        var numRows = jq('#jobsTable tbody').find('tr').length;
            
                        if (numRows > minJobs) {
                            try {
                                jq.fn.dataTableExt.oSort['posted-asc'] = function(daysAgoTextA, daysAgoTextB) {
                                    return sortByDaysAgo(-1, daysAgoTextA, daysAgoTextB);
                                };
            
                                jq.fn.dataTableExt.oSort['posted-desc'] = function(daysAgoTextA, daysAgoTextB) {
                                    return sortByDaysAgo(1, daysAgoTextA, daysAgoTextB);
                                };
            
                                var daysPostedColumnExists = 'https:\/\/www.ziprecruiter.com\/jobs-widget-content\/v1\/889f3581\/all'.toString().indexOf('show_posted_days=0') == -1;
            
                                var domStr = numRows > jobsPerPage ? 'ftp' : numRows > minSearch ? 'ft' : 't';
                                jq('#jobsTable').DataTable({
                                    'dom': domStr,
                                    'pageLength': jobsPerPage,
                                    'aaSorting': [],
                                    'aoColumns': daysPostedColumnExists ? [null, null, {
                                        'sType': 'posted'
                                    }] : [null, null],
                                    'oLanguage': {
                ...
                function prepareRequest(params) {
                    ...
                    if (params.forceXdomainPlugin || (zrHead.browser.ie && zrHead.browser.version <= 9) || (ieUserAgent.compatibilityMode && ieUserAgent.renderVersion <= 9)) {
                        zrlog("zr jobs widget - old IE or old IE compatibility mode");
                        zrHead.load("https://www.ziprecruiter.com/js/vendor/jquery-migrate/1.2.1/jquery-migrate-1.2.1.js", "https://www.ziprecruiter.com/js/vendor/jquery-ajaxtransport-xdomainrequest/1.0.1/jquery.xdomainrequest.min.js", go);
                    } else {
                ...
                if ((jqVersionNums.major == 1 && jqVersionNums.minor >= 9) || (jqVersionNums.major >= 2)) {
                    zrlog("zr jobs widget - jquery exists");
                    doAjaxReq();
                } else {
                    zrlog("zr jobs widget - loading custom jquery");
                    zrHead.load("https://www.ziprecruiter.com/js/vendor/jquery/1.9.1/jquery.js", function() {
                        prepareRequest({
                            customJquery: true
                        });
                    });
