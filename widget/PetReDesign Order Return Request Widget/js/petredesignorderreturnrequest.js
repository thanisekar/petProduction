    ...
    return {
      ...
      restrictNumber: function() {
        ...
        onLoad : function(widget) {
            getWidget = widget;
          //  //console.log("WIDGET", widget.returnReason());
           // //console.log("Return POST URL", widget.returnPostURL());
            widget.reasonArray(widget.returnReason().split(","));
          //  //console.log("Reason Array+++++", widget.reasonArray());
             ...
             //   //console.log("Return Request",returnRequest);
              ...
              //  //console.log("Reason Array value+++++", widget.reasonArray());
        },  
      ...
      beforeAppear: function (page) {
        ...
        //console.log("ORDER DETAILS+++++++", getWidget.orderDetails());
        /*Order Return PubSub*/
         ...
         $.Topic("returnRequest").subscribe(function(data){
                ...
                $("#returnRequest").show();
                getWidget.returnListItems.push(getWidget.returnReq().items);
      ...
      returnRequestForm : function(data) {
            ...
            var err = $('.product--return').find($('.error'));
               ...
               // var getDivId;
                $.each($('.order-Items-return'), function(){
                 //   //console.log($(this).id +'----order return id ---');
            ...
            requestJSON.returnRequest.orderId = getWidget.orderDetails().id;
            //requestJSON.returnRequest.shippingEmail = getWidget.orderDetails().shippingAddress.email;
            requestJSON.returnRequest.returnItemsList = [];
            //requestJSON.returnRequest.totalRequestCount = len;
            var skuIDs = [];
            var quant = [];
            var reasonss = [];
            var maxQuanty = [];
            ...
            $(".product-quantity-input--return").each(function(){
              ...
              /*$.each($('.product--return'), function(){
                   var maxQty=$(this).find('.Qty').attr('maxQty');
                   //console.log(maxQty+ "//////////// max");
                   var item = [];
                   item['maxQuantity']=maxQty;
                   maxQuanty.push(item);
                   //console.log(maxQuanty,'inside each in maxqty')
              });*/
            for(var i = 0; i<len; i++) {
           ...
           // //console.log('deleteData', requestJSON.returnRequest.returnItemsList);
           // //console.log('post delte', requestJSON.returnRequest.returnItemsList);
           // //console.log()
            
            /*for(var i=0; i<requestJSON.returnRequest.returnItemsList.length; i++) {
                //console.log("camel case values",widget.reasonArray(tempReasonArrayVal));
                requestJSON.returnRequest.returnItemsList[i].returnReason = requestJSON.returnRequest.returnItemsList[i].returnReason.toLowerCase();
                //requestJSON.returnRequest.returnItemsList[i].returnReason=widget.reasonArray(tempReasonArrayVal);
            }*/
            var db = {};
