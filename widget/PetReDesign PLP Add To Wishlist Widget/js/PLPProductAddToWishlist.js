...
function($, ko, PubSub, notifier, logger, swmRestClient, swmKoValidateRules, placeholder, CCConstants, CCi18n) {
  ...
  return {
    ...
    handleAddToSpace: function() {
      ...
      //console.log('widget.site()',widget.site())
            //console.log('widget.site().selectedPriceListGroup().currency.symbol',widget.site().selectedPriceListGroup().currency.symbol)
            //console.log('widget.productToAdd.childSKUs[0].listPrice',widget.productToAdd[0].childSKUs[0].listPrice)
            //console.log('fractionalDigits',widget.site().selectedPriceListGroup().currency.fractionalDigits)

      if (widget.productToAdd[0] === null) {
        alert('no product!');
        return;
      }
      ...
      var successCB = function(result) {
        if (result.response.code.indexOf("200") === 0 || result.response.code.indexOf("201") === 0) {
          ...
          $.Topic(PubSub.topicNames.SOCIAL_SPACE_ADD_SUCCESS).publishWith(widget.productToAdd[0], [addToSpaceData]);
          ...
          if (!productUpdated && widget.productToAdd[0].primaryFullImageURL.lastIndexOf(".") > 0) {
            var fileExt = widget.productToAdd[0].primaryFullImageURL.slice(widget.productToAdd[0].primaryFullImageURL.lastIndexOf("."));
            var prodImg = new Image();
            prodImg.src = widget.productToAdd[0].primarySmallImageURL;
      ...
      if (spaceid == "createnewspace") {
        ...
        var createSpaceSuccessCB = function(result) {        
          if (result.response.code.indexOf("201") === 0) {
            ...
            if (widget.productToAdd[0].childSKUs[0].salePrice != null) {
              productToAddSalePrice = JSON.stringify(widget.productToAdd[0].childSKUs[0].salePrice);
            }

            var productTitleToAdd = widget.productToAdd[0].displayName ? widget.productToAdd[0].displayName : '';
            
            var json = {
                // instantiate minimum payload
                productTitle: productTitleToAdd,
                productPrice: widget.productToAdd[0].childSKUs[0].listPrice,
                productSalePrice: productToAddSalePrice,
                productUrl: widget.productToAdd[0].route,
                productProductId: widget.productToAdd[0].id,
                productSkuId: widget.productToAdd[0].childSKUs[0].repositoryId,
                ...
                notificationProductPrice : widget.site().selectedPriceListGroup().currency.symbol + widget.formatPrice(widget.productToAdd[0].childSKUs[0].listPrice, widget.site().selectedPriceListGroup().currency.fractionalDigits)
                //notificationProductPrice : widget.site().selectedPriceListGroup().currency.symbol + widget.productToAdd.childSKUs[0].listPrice
            };
            
            if (widget.productToAdd[0].primarySmallImageURL && widget.productToAdd[0].primarySmallImageURL != 'no-image') {
              // if there is a primarySmallImageURL in ProductViewModel AND primarySmallImageURL value is not 'no-image', then include productImageUrl in request payload.
            	json.productImageUrl = window.location.protocol + "//" + window.location.host + widget.productToAdd[0].primarySmallImageURL;
            }
            ...
            if (json.productSalePrice) {
              ...
              json.notificationProductPrice = widget.site().selectedPriceListGroup().currency.symbol + widget.formatPrice(widget.productToAdd[0].childSKUs[0].salePrice, widget.site().selectedPriceListGroup().currency.fractionalDigits);
             //json.notificationProductPrice = widget.site().selectedPriceListGroup().currency.symbol + widget.productToAdd.childSKUs[0].salePrice;
            }
        ...
        var addToSpaceData = {
          displayName: widget.productToAdd[0].displayName,
          spaceName: $('#SWM-addToSpace-name')[0].value,
          productId: widget.productToAdd[0].id
        };
        ...
        var json = {
            productTitle: productTitleToAdd,
            productPrice: JSON.stringify(widget.productToAdd[0].childSKUs[0].listPrice),
            productSalePrice: productToAddSalePrice,
            productUrl: widget.productToAdd[0].route,
            productProductId: widget.productToAdd[0].id,
            productSkuId: widget.productToAdd[0].childSKUs[0].repositoryId,
