// REST library is designed to connect to an Oracle Cloud Commerce (occ) from a Node.JS application.
...
/**
 * @class
 * This constructor function creates an occAPI instance.
 * <p>
 ...
 *        throw an error.
 * @returns {CommerceSDK} The occ instance
 */
...
CommerceSDK.prototype.request = function (args) {
	...
	var currentRequest = requestModule.request (requestOptions, function (response) {
		...
		response.on('end', function() {
			CommerceSDK.printDebugMessage('The request is complete.' + responseData);
			...
			if (CommerceSDK.isErrorResponse(jsonResponseData) === true) {
				...
				//args.callback ('There was an error in the response.', jsonResponseData);
				 args.callback (jsonResponseData, jsonResponseData);
			} else {
...
CommerceSDK.prototype.getAccessToken = function (urlOptions) {
	'use strict';
	var self = this;
	if (!urlOptions) {
		throw new Error ('The url options cannot be null');
	...
	initPromise.then (function (success) {
		if (success === true) {
			// urlOptions.data = JSON.stringify(urlOptions.data);
			console.log("Final tken"+self.accessToken);
			 urlOptions.callback( null, self.accessToken);
		}
		else{
			urlOptions.callback(null, null);
		}
	});
};
